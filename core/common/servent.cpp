...
void Servent::handshakeIn() {
    ...
    //if (!priorityConnect)
    {
        if (!isPrivate()) if (servMgr->pubInFull())
            throw HTTPException(HTTP_SC_UNAVAILABLE, 503);
    }
    ...
    while (thread.active && sock->active()) {
        ...
        if (sock->readReady()) {
            ...
            if (gnuStream.readPacket(pack)) {
                ...
                if (pack.func != GNU_FUNC_PONG) if (servMgr->seenPacket(pack))
        ...
        if (chanMgr->isBroadcasting() && servMgr->autoServe) {
            ...
            try {
                ...
                while (!error && sv->thread.active && !sv->sock->eof() && servMgr->autoServe) {
                    ...
                    if (sv->pcpStream->nextRootPacket) if (sys->getTime() > (sv->pcpStream->nextRootPacket + 30))
                    ...
                    if (rawPack.type == ChanPacket::T_DATA) {
                        ...
                        while (len) {
                            ...
                            if (bufPos >= interval) {
                                ...
                                if (chanMgr->broadcastMsgInterval) if ((sys->getTime() - lastMsgTime) >= chanMgr->broadcastMsgInterval) {
                                    showMsg ^= true;
