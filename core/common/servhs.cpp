...
void Servent::handshakeHTTP(HTTP &http, bool isHTTP) {
    ...
    if (http.isRequest("GET /")) {
        ...
        if (strncmp(fn, "/admin?", 7) == 0) {
            ...
            if (!sock->host.isLocalhost()) if (!isAllowed(ALLOW_DIRECT) || !isFiltered(ServFilter::F_DIRECT))
                throw HTTPException(HTTP_SC_UNAVAILABLE, 503);
            ...
            if (!sock->host.isLocalhost()) if (!isAllowed(ALLOW_NETWORK) || !isFiltered(ServFilter::F_NETWORK))
                throw HTTPException(HTTP_SC_UNAVAILABLE, 503);
...
// -----------------------------------
void Servent::handshakePLS(ChanHitList * *cl, int
num,
bool doneHandshake
)
{
