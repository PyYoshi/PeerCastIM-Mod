...
bool ServMgr::seenHost(Host &h, ServHost::TYPE type, unsigned int time) {
    ...
    for (int i = 0; i < MAX_HOSTCACHE; i++)
        if (hostCache[i].type == type) if (hostCache[i].host.ip == h.ip) if (hostCache[i].time >= time)
            return true;
    return false;
}
...
void ServMgr::addHost(Host &h, ServHost::TYPE type, unsigned int time) {
    ...
    for (i = 0; i < MAX_HOSTCACHE; i++)
        if (hostCache[i].type == type) if (hostCache[i].host.isSame(h)) {
            sh = &hostCache[i];
            break;
        }
...
void ServMgr::deadHost(Host &h, ServHost::TYPE t) {
    for (int i = 0; i < MAX_HOSTCACHE; i++)
        if (hostCache[i].type == t) if (hostCache[i].host.ip == h.ip) if (hostCache[i].host.port == h.port)
            hostCache[i].init();
}
...
int ServMgr::getNewestServents(Host *hl, int max, Host &rh) {
    ...
    for (int i = 0; i < max; i++) {
        ...
        for (int j = 0; j < MAX_HOSTCACHE; j++) {
            // find newest servent
            if (hostCache[j].type == ServHost::T_SERVENT) if (!(rh.globalIP() && !hostCache[j].host.globalIP())) {
...
Servent *ServMgr::findOldestServent(Servent::TYPE type, bool priv) {
    ...
    Servent *s = servents;
    while (s) {
        if (s->type == type) if (s->thread.active) if (s->isOlderThan(oldest)) if (s->isPrivate() == priv)
            oldest = s;
        s = s->next;
    }
    return oldest;
...
void    ServMgr::closeConnections(Servent::TYPE type) {
    Servent *sv = servents;
    while (sv) {
        if (sv->isConnected()) if (sv->type == type)
...
unsigned int ServMgr::numConnected(int type, bool priv, unsigned int uptime) {
    ...
    Servent *s = servents;
    while (s) {
        if (s->thread.active) if (s->isConnected()) if (s->type == type) if (s->isPrivate() == priv) if ((ctime - s->lastConnect) >= uptime)
            cnt++;
...
unsigned int ServMgr::numConnected() {
    ...
    Servent *s = servents;
    while (s) {
        if (s->thread.active) if (s->isConnected())
            cnt++;
...
unsigned int ServMgr::totalOutput(bool all) {
    ...
    Servent *s = servents;
    while (s) {
        if (s->isConnected()) if (all || !s->isPrivate()) if (s->sock)
...
bool ServMgr::seenPacket(GnuPacket & p) {
    Servent *s = servents;
    while (s) {
        if (s->isConnected()) if (s->seenIDs.contains(p.id))
            return true;
        s = s->next;
    }
    return false;
...
int ServMgr::broadcast(GnuPacket & pack, Servent * src) {
    ...
    if (pack.ttl) {
        ...
        while (s) {
            ...
            if (s != src) if (s->isConnected()) if (s->type == Servent::T_PGNU) if (!s->seenIDs.contains(pack.id)) {

                if (src) if (!src->networkID.isSame(s->networkID))
                    continue;
...
int ServMgr::route(GnuPacket & pack, GnuID & routeID, Servent * src) {
    ...
    if (pack.ttl) {
        Servent *s = servents;
        while (s) {
            if (s != src) if (s->isConnected()) if (s->type == Servent::T_PGNU) if (!s->seenIDs.contains(pack.id)) if (s->seenIDs.contains(routeID)) {
                if (src) if (!src->networkID.isSame(s->networkID))
                    continue;
...
bool ServMgr::isFiltered(int fl, Host &h) {
    for (int i = 0; i < numFilters; i++)
        if (filters[i].flags & fl) if (h.isMemberOf(filters[i].host))
            return true;
...
unsigned int ServMgr::numStreams(GnuID &cid, Servent::TYPE tp, bool all) {
    ...
    Servent *sv = servents;
    while (sv) {
        if (sv->isConnected()) if (sv->type == tp) if (sv->chanID.isSame(cid)) if (all || !sv->isPrivate())
            cnt++;
        sv = sv->next;
    }
    return cnt;
...
unsigned int ServMgr::numStreams(Servent::TYPE tp, bool all) {
    ...
    Servent *sv = servents;
    while (sv) {
        if (sv->isConnected()) if (sv->type == tp) if (all || !sv->isPrivate()) {
...
Servent *ServMgr::findConnection(Servent::TYPE t, GnuID &sid) {
    Servent *sv = servents;
    while (sv) {
        if (sv->isConnected()) if (sv->type == t) if (sv->remoteID.isSame(sid))
            return sv;
        sv = sv->next;
    }
    return NULL;
...
bool ServMgr::isCheckPushStream() {
    if (servMgr->kickPushStartRelays) if (servMgr->numStreams(Servent::T_RELAY, false) >= (servMgr->kickPushStartRelays - 1))
        return true;
