...
public:
    MemoryStream()
            : buf(NULL), own(false), len(0), pos(0) {
    }

    MemoryStream(void *p, int l)
            : buf((char *) p), own(false), len(l), pos(0) {
    }

    MemoryStream(int l)
            : buf(new char[l]), own(true), len(l), pos(0) {
    }
...
public:
    WriteBufferStream(Stream *out_)
            : buf(NULL), own(false), len(0), pos(0), out(out_) {
    }

    WriteBufferStream(void *p, int l, Stream *out_)
            : buf((char *) p), own(false), len(l), pos(0), out(out_) {
    }

    WriteBufferStream(int l, Stream *out_)
            : buf(NULL), own(true), len(l), pos(0), out(out_) {
        buf = (char *) malloc(l);
        if (!buf)
