...
enum {
    kChannelColumn = 'CHAN', kBitRateColumn = 'BITR', kStreamColumn = 'STRM', kRelayColumn = 'TRLY', kListenersColumn = 'LRLY', kStatusColumn = 'STAT', kKeepColumn = 'KEEP', kNumProps = 7
};

enum {
    kEventClassPeercast = 'PEER', kEventStartChannel = 'STRT', kEventUpdateChannel = 'UPDT', kEventStopChannel = 'STOP', kEventParamChanStatus = 'CSTA', kEventParamSystem = 'PSYS', typeChanStatus = 'CSTA'
};

UInt32 propIDList[kNumProps] = {
        kChannelColumn, kBitRateColumn, kStreamColumn, kRelayColumn, kListenersColumn, kStatusColumn, kKeepColumn
};
...
OSXPeercastApp::OSXPeercastApp(const char *appPath, const char *iniFilename, int argc, char *argv[], WindowRef window, const ControlID &dbControlID)
        : mIniFileName(),
...
OSStatus OSXPeercastApp::installAppEventHandler() {
    ...
    return InstallApplicationEventHandler(mAppEventHandler.eventHandler(), GetEventTypeCount(appEvents), appEvents, NULL, NULL);
}
...
OSStatus OSXPeercastApp::installWindowHandler(WindowRef window) {
    static const EventTypeSpec mainWindowEvents[] =
            {
                    {kEventClassCommand, kEventCommandProcess}, {kEventClassWindow, kEventWindowActivated}
            };

    return InstallWindowEventHandler(window, mMainWindowHandler.eventHandler(), GetEventTypeCount(mainWindowEvents), mainWindowEvents, this, NULL);
}
...
OSStatus OSXPeercastApp::appEventHandler(EventHandlerCallRef inCallRef, EventRef inEvent, void *inUserData) {
    ...
    OSStatus err = GetEventParameter(inEvent, kEventParamDirectObject, typeHICommand, NULL, sizeof(HICommand), NULL, &command);
...
OSStatus OSXPeercastApp::installPeercastEventHandler(WindowRef theWindow) {
    static const EventTypeSpec peercastEventTypes[] =
            {
                    {kEventClassPeercast, kEventStartChannel}, {kEventClassPeercast, kEventUpdateChannel}, {kEventClassPeercast, kEventStopChannel}
            };

    return InstallEventHandler(GetApplicationEventTarget(), mPeercastEventHandler.eventHandler(), GetEventTypeCount(peercastEventTypes), peercastEventTypes, this, NULL);
}
