...
OSErr gurlHan(const AppleEvent *theAppleEvent, AppleEvent *reply, SInt32 handlerRefcon) {
    ...
    // Get the event ID
    err = AEGetAttributePtr(theAppleEvent, keyEventIDAttr, typeType, &typeCode, (Ptr) &eventID, sizeof(eventID), &actualSize);
...
OSStatus runApp(int argc, char *argv[], WindowRef window) {
    ...
    Timer relayBrowserTimer(GetMainEventLoop(), 3 * kEventDurationSecond, kEventDurationSecond, pOSXApp, relayBrowserUpdate);
    //////////////////////////
